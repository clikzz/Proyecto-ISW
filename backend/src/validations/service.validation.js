...
const createServiceSchema = Joi.object({
  name_service: Joi.string().max(255).required(),
  description_service: Joi.string().max(500).optional(),
  price_service: Joi.number().positive().required(),
  date_service: Joi.date().iso().required(),
  category: Joi.string().valid('Reparación', 'Mantención', 'Personalización').required(),
  user_rut: Joi.string().max(12).required(),
